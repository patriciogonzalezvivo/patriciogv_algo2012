...
void TimeLine::draw(){
    ...
    if (points.size() > 0){
        ...
        if ( isFinish() ){
            ...
            float timeToCheck = ofGetElapsedTimef() - points[points.size()-1].time;
			...
			ofRect(0,0,50 + vel.length()*0.2,50);			// this does some stretching based on velocity.
//            ofCircle(0,0, 2 + vel.length()/5.0);
            ofPopMatrix();
        }
    }
}
