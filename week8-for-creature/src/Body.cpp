...
bool Body::load( string _file, ofPoint _initPos, float _scale ){
    scale = _scale;
...
void Body::restart(){
    for(int i = 0; i < vertices.size(); i++){
        ...
        float radio = toCenter.length() * scale;
...
Vertex* Body::operator [] (int _index){
...
bool Body::addSpring(unsigned int _from, unsigned int _to, float _k ){
    ...
    if ( (_from < vertices.size()) && (_to < vertices.size()) && (_to != _from) ){
        ...
        newSpring.dist      = vertices[ _from]->distance( *vertices[ _to]);
...
void Body::collideWith( Body *_body ){
    for (int i = 0; i < vertices.size(); i++){
        for (int j = 0; j < _body->size(); j++ ){
            vertices[i]->addRepulsionForce( (*_body)[j] , 20, 1.0);
        }
    }
}
...
void Body::draw(){
    ...
    if (bDebug){
        ...
        ofSetColor(0,80);
        for (int i = 0; i < springs.size(); i++){
            ofLine( *springs[i].vertexA, *springs[i].vertexB);
        }
