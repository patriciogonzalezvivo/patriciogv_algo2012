...
void testApp::setup(){
    ...
    pallete.allocate(palleteImg.getWidth(), palleteImg.getHeight(), GL_RGBA);
    pallete.begin();
    ofClear(0,0);
    ofSetColor(255);
    palleteImg.draw(0, 0);
...
void testApp::mouseDragged(int x, int y, int button){
    if (bPallete){
        ...
        ofDisableAlphaBlending();
        pallete.end();
        brush.pickColorFrom( pallete.getTextureReference(),0.9,0.01 );
    }
    brush.set(x,y);
}
...
void testApp::mousePressed(int x, int y, int button){
    ...
    if (bPallete){
        ...
        ofDisableAlphaBlending();
        pallete.end();
        brush.pickColorFrom( pallete.getTextureReference(), 1.0, 0.01 );
    }
